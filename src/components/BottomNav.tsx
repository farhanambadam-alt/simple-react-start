...
const BottomNav = () => {
  ...
  const updateBottomNavClearance = useCallback(() => {
    const root = document.documentElement;

    if (hidden || !navRef.current) {
      root.style.setProperty('--bottom-nav-clearance', '0px');
      return;
    }

    const navTop = navRef.current.getBoundingClientRect().top;
    const clearance = Math.max(window.innerHeight - navTop, 0);
    root.style.setProperty('--bottom-nav-clearance', `${Math.round(clearance)}px`);
  }, [hidden]);
  ...
  useEffect(() => {
    ...
    window.addEventListener('resize', updateBottomNavClearance);
    return () => {
      window.removeEventListener('resize', updateSpotlight);
      window.removeEventListener('resize', updateBottomNavClearance);
      document.documentElement.style.setProperty('--bottom-nav-clearance', '0px');
    };
  }, [updateSpotlight, updateBottomNavClearance]);
