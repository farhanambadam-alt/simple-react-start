...
import { Star, CheckCircle2, ThumbsUp, ChevronLeft, ChevronRight, Trash2 } from 'lucide-react';
...
interface ReviewsSectionProps {
  artists: Artist[];
  reviews: (Review | ReviewWithPhotos)[];
  selectedArtist: string | null;
  onSelectArtist: (id: string | null) => void;
  /** If provided, renders a delete button on before/after photos (staff mode) */
  onDeletePhoto?: (reviewId: string) => void;
  /** Hide the artist selector strip (e.g. when used in staff profile) */
  hideArtistSelector?: boolean;
  /** Override title */
  title?: string;
}

const ReviewsSection = ({ artists, reviews, selectedArtist, onSelectArtist, onDeletePhoto, hideArtistSelector, title }: ReviewsSectionProps) => {
  ...
  return (
    <div className="animate-fade-in-up" style={{ animationDuration: '250ms' }}>
      {/* Stylists Header - conditionally hidden */}
      {!hideArtistSelector && (
      <div className="px-5 pt-5 pb-1">
        <div className="flex items-baseline justify-between mb-4">
          <h2 className="font-serif text-[20px] text-foreground italic">{title || 'Our Stylists'}</h2>
        ...
        <div className="space-y-2 px-2.5 pb-3 md:grid md:grid-cols-2 md:gap-2 md:space-y-0">
          {filteredReviews.map((review, index) => (
            ...
            >
              ...
              {/* Before/After gradient pair */}
              {('beforeImage' in review || 'afterImage' in review) && ((review as ReviewWithPhotos).beforeImage || (review as ReviewWithPhotos).afterImage) && (
                <div className="mt-3 rounded-xl overflow-hidden ring-1 ring-border relative">
                  <div className="flex">
                    <div className="flex-1 aspect-[4/3] bg-gradient-to-br from-secondary to-muted flex flex-col items-center justify-center">
                      <span className="text-[9px] font-heading font-bold uppercase tracking-wider text-muted-foreground bg-background/60 px-2 py-0.5 rounded-full">Before</span>
                    </div>
                    <div className="w-px bg-border" />
                    <div className="flex-1 aspect-[4/3] bg-gradient-to-br from-primary/10 to-primary/5 flex flex-col items-center justify-center">
                      <span className="text-[9px] font-heading font-bold uppercase tracking-wider text-primary/70 bg-background/60 px-2 py-0.5 rounded-full">After</span>
                    </div>
                  </div>
                  {onDeletePhoto && (
                    <button
                      onClick={() => onDeletePhoto(review.id)}
                      className="absolute top-2 right-2 p-1.5 rounded-full bg-background/80 text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors"
              ...
              {review.hasPhoto && !('beforeImage' in review) && (
