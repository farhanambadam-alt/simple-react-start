...
import { Heart, HelpCircle, LogOut, ChevronRight, Edit3, Camera, ArrowLeft, Star, MapPin, Trash2, FileText, Shield, RotateCcw, Mail, Info } from 'lucide-react';
    ...
    <div className="min-h-screen pb-safe">
      ...
      <div className="px-5 space-y-2.5 max-w-7xl mx-auto">
        ...
        <div className="pt-6 pb-1">
          <p className="px-1 mb-2 text-[11px] font-heading font-semibold tracking-widest uppercase text-muted-foreground">
            Legal & Support
          </p>
          <div className="bg-card rounded-2xl border border-border card-shadow overflow-hidden divide-y divide-border">
            {[
              { icon: FileText, label: 'Terms & Conditions', to: '/terms' },
              { icon: Shield, label: 'Privacy Policy', to: '/privacy' },
              { icon: RotateCcw, label: 'Cancellation & Refund', to: '/refund-policy' },
              { icon: Mail, label: 'Contact Us', to: '/contact' },
            ].map((row) => (
              <button
                key={row.to}
                onClick={() => navigate(row.to)}
                className="w-full flex items-center gap-3.5 p-4 active:bg-muted/40 transition-colors min-h-[56px] text-left"
              >
                <div className="w-9 h-9 rounded-xl bg-primary/8 flex items-center justify-center">
                  <row.icon size={18} className="text-primary" />
                </div>
                <span className="flex-1 font-heading font-medium text-[14px] text-foreground">{row.label}</span>
                <ChevronRight size={18} className="text-muted-foreground/60" />
              </button>
            ))}
          </div>
          <p className="text-center text-[11px] font-body text-muted-foreground mt-3 flex items-center justify-center gap-1.5">
