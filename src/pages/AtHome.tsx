  ...
  const headerLeft = (
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        <div className="flex items-center gap-1.5 mt-1 max-w-[200px]">
          <MapPin size={12} className="text-primary shrink-0" />
          <span className="text-[12px] font-body text-muted-foreground truncate">{location.areaName ? `${location.areaName}, ${location.cityName}` : location.cityName}</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen relative pb-safe">
      ...
      <div className="px-5 mt-4 max-w-7xl mx-auto">
        <MiracleSearchBar placeholder="Search artists, services" />
      </div>
      ...
      {/* Content */}
      {hasArtists ? null : (
        <div className="px-5 pt-8 flex flex-col items-center text-center max-w-7xl mx-auto">
          <MapPin size={32} className="text-muted-foreground/50 mb-4" />
