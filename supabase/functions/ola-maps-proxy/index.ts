...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "reverse-geocode") {
      ...
      // Log address components for debugging street-level accuracy
      const results = data?.results || [];
      for (let i = 0; i < Math.min(results.length, 3); i++) {
        const r = results[i];
        const types = (r.address_components || []).map((c: any) => `${c.types?.[0]}=${c.long_name}`).join(' | ');
        console.log(`Result[${i}] formatted: ${r.formatted_address?.substring(0, 100)}`);
        console.log(`Result[${i}] components: ${types.substring(0, 300)}`);
      }
